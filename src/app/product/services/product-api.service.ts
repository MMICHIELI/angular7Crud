...
import { IPage, IPageRequest } from 'src/app/core/models/pagination.model';
...
export class ProductApiService extends ApiService {
    ...
    public getProducts(productPageRequest?: IPageRequest): Observable<IPage<Product>> {

        const params: HttpParams = new HttpParams();
        params.set('size', `${productPageRequest.size}`);
        params.set('page', `${productPageRequest.page}`);
        params.set('sortProperty', `${productPageRequest.sortProperty}`);
        params.set('sortDirection', `${productPageRequest.sortDirection}`);

        return this.http.get<IGenericResponse<IPage<Product>>>(this.apiProduct, { params })
